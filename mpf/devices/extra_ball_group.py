...
class ExtraBallGroup(SystemWideDevice):
    ...
    def extra_ball_initialized(self, extra_ball):
        if extra_ball.name in self.config['extra_balls']:
            self.debug_log("Adding '{}' to this group".format(extra_ball))
            self.extra_balls.add(extra_ball)

    def ok_to_award_eb(self):
        if not self.player:
            return False
    ...
    def _player_added(self, player, **kwargs):
        ...
        player['extra_ball_group_{}_total_awarded'.format(self.name)] = 0
        player['extra_ball_group_{}_lit'.format(self.name)] = 0
        player['extra_ball_group_{}_current_ball_awarded'.format(self.name)] = 0
    ...
    def _player_turn_start(self, player, **kwargs):
        ...
        if player['extra_ball_group_{}_lit'.format(self.name)]:
            self.relight()
    ...
    def award_lit_extra_ball(self, **kwargs):
        ...
        if self.player['extra_ball_group_{}_lit'.format(self.name)] < 1:
            return

        self.player['extra_ball_group_{}_lit'.format(self.name)] -= 1
        self.award_extra_ball()

        if not self.player['extra_ball_group_{}_lit'.format(self.name)]:
            self.machine.events.post('extra_ball_group_{}_unlit'.format(self.name))
    ...
    def award_extra_ball(self, **kwargs):
        ...
        # todo figure out what haapens if the group is awarded, vs, the individual EB
        # and which things should be tracked where.

        # when an EB is lit, that should count as awarded? Or 
        ...
        if (self.config['max_per_ball'] and
                self.player['extra_ball_group_{}_current_ball_awarded'.
                        format(self.name)] >= self.config['max_per_ball']):

            self.machine.events.post('extra_ball_group_{}_max_exceeded'.format(self.name))
            '''event: extra_ball_group_(name)_max_exceeded

            desc: The configured max extra balls (either for this
            ball or total for the game) for this player for this extra ball
            group has been exceeded, so this event is posted instead of the
            extra ball awarded event.

            '''

        elif (self.config['max_per_game'] and
                self.player['extra_ball_group_{}_total_awarded'.format(self.name)] >= self.config['max_per_game']):
            self.machine.events.post('extra_ball_group_{}_max_exceeded'.format(self.name))
            ...
            if not self.events_only:
                self.player['extra_ball_group_{}_total_awarded'.format(self.name)] += 1
                self.player['extra_ball_group_{}_current_ball_awarded'.format(self.name)] += 1
                self.player.extra_balls += 1

    def light_extra_ball(self, **kwargs):
